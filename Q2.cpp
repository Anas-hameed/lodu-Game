...
#define TurnConsective 4  // this is Max limit for player who got cancel if he does not rolls six or don't kill other Player
...
struct Board
{
	...
	void DisplayPlayerInfo()
	{
		...
		for(int i=0; i<Players; i++)
		{
			DrawString(200,y,"PLAYER NAMES:: "+playerName[i],colors[RED]);
			y-=50;
		}
	...
	void DrawEndScreen()
	{
		...
		for(int i=0; i<Players; i++)
		{
			...
			str+= Num2Str(Player_s[i].PlayerID);
			str+= "   :: ";
			if(Player_s[i].concelstate==1)
			str+= "Concel(last)";
			else
			{
				int c=0;
	...
	while(score>0)
	{
		if(score<6)
		{
			...
			{	
				...
				else// this is check for double role of same goti if it has hitrate= 0
				{
					cout<<"I am not Giving you Segmentation Fault"<<endl;
					b.Player_s[playerNo].Tokens[index].Index=(temp+6)%RouteSize;
					CurScore=0;
				}
				HitCheck(playerNo);
...
bool Cancellation_Check(int No)
{
	...
	{
		count++;
		b.Player_s[No].concelstate=0;// Concel Without Error
		return check;
	}
	...
	{
		count++;
		b.Player_s[No].concelstate=1;//Cancel With Error
		return true;
	}
	if(count==3)
	check=true;
...
void* PlayerTurn(void* arg)
{
	...
	while(true)
	{
		...
		b.Player_s[playerNo].Noofturns++;
		c1= Cancellation_Check(playerNo);
		if(b.Player_s[playerNo].Noofturns==TurnConsective)
		{
			b.Player_s[playerNo].Noofturns=0;
			b.Player_s[playerNo].SixCount=0;
		}
		p1= playerNo;
		playerNo=-1;
		sem_post(&Grid_Acess);
		...
		{
			totalp--;
			b.Player_s[p1].PlayerID = pthread_self();
		}
		cout<<c1<<"  "<<p1<<endl;
		sem_post(&RoundTurns);
		To_Cancel[p1]= c1;
...
void performCheck(int gotiNo)
{
	...
	if(index!=-1)
	{
		...
		else// This if make the token to complete round if hitrate is zero 
		{
			b.Player_s[playerNo].Tokens[gotiNo].Index=(index+6+CurScore)%RouteSize;
			CurScore=0;
		}
